...
namespace DuiLib
{
	...
	Gdiplus::Image* CGifAnimUI::LoadGifFromFile(LPCTSTR pstrGifPath)
	{
		...
		DUITRACE(_T("CGifAnimUI::LoadGifFromFile, pstrGifPath=%s"), pstrGifPath);
		...
		do 
		{
			...
			if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
				sFile += pstrGifPath;
				DUITRACE(_T("CGifAnimUI::LoadGifFromFile, sFile=%s"), sFile.GetData());
		...
		DUITRACE(_T("CGifAnimUI::LoadGifFromFile, pData=0x%p"), pData);
		while (!pData)
		{
