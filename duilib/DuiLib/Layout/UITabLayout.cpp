...
namespace DuiLib
{
	...
	{
		DUITRACE(_T("CTabLayoutUI::Add. Item=0x%p,  class=%s"), pControl, pControl->GetClass());
	...
	{
		DUITRACE(_T("CTabLayoutUI::AddAt. Item=%p, Index=%d"), pControl, iIndex);
	...
	bool CTabLayoutUI::SelectItem(int iIndex,  bool bTriggerEvent)
	{
		...
		{
			DUITRACE(_T("CTabLayoutUI::SelectItem. Item=%p"), GetItemAt(it));
			if( it == iIndex ) {
