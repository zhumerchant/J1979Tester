...
const int kPlayListItemNormalHeight = 24;
const int kPlayListItemSelectedHeight = 24;


CPlayList::CPlayList(CPaintManagerUI& paint_manager)
	: root_node_(NULL)
	, delay_deltaY_(0)
	, delay_number_(0)
	, delay_left_(0)
	...
	, paint_manager_(paint_manager)
{
	...
	root_node_->data().playing_ =false;
...
bool CPlayList::Add(CControlUI* pControl)
{
	if (!pControl)
		return false;
...
bool CPlayList::AddAt(CControlUI* pControl, int iIndex)
{
	if (!pControl)
		return false;
...
bool CPlayList::Remove(CControlUI* pControl)
{
	if (!pControl)
		return false;
...
bool CPlayList::RemoveAt(int iIndex)
{
...
const TCHAR* const kDescriptionControlName = _T("full_filename"); 
...
Node* CPlayList::AddNode(const PlayListItemInfo& item, Node* parent)
{
	if (!parent)
		parent = root_node_;
	...
	{
        pListElement = static_cast<CListContainerElementUI*>(m_dlgBuilder.Create(_T("play_list_item.xml"), (LPCTSTR )0, NULL, &paint_manager_));
    }
    else 
	{
        pListElement = static_cast<CListContainerElementUI*>(m_dlgBuilder.Create((IDialogBuilderCallback *)0, &paint_manager_));
    }
	...
	if (log_button != NULL)
	{
					...
					_stprintf(szBuf, _T("%s"), item.logo.GetData());
		#else
					_sntprintf(szBuf, MAX_PATH - 1, _T("%s"), item.logo.GetData());
		#endif
					log_button->SetNormalImage(szBuf);

					if (item.playing  && !item.logo.IsEmpty())
		{
...
#else
		_sntprintf(szBuf, MAX_PATH - 1, _T("<x %d>"), level_text_start_pos_);
#endif
		html_text += szBuf;
	}
	...
	{
#if defined(UNDER_WINCE)
		_stprintf(szBuf, _T("%s"), item.file_name.GetData());
#else
		_sntprintf(szBuf, MAX_PATH - 1, _T("%s"), item.file_name.GetData());
#endif
		html_text += szBuf;
	}

	CLabelUI* file_name = static_cast<CLabelUI*>(paint_manager_.FindSubControlByName(pListElement, kNickNameControlName));
	if (file_name != NULL)
	{
		if (item.folder)
			file_name->SetFixedWidth(0);

		file_name->SetShowHtml(true);
		file_name->SetText(html_text.GetData());
	}

	if (!item.folder && !item.full_filename.IsEmpty())
	{
		CLabelUI* full_filename = static_cast<CLabelUI*>(paint_manager_.FindSubControlByName(pListElement, kDescriptionControlName));
		if (full_filename != NULL)
		{
#if defined(UNDER_WINCE)
			_stprintf(szBuf, _T("<x 20><c #808080>%s</c>"), item.full_filename.GetData());
#else
			_sntprintf(szBuf, MAX_PATH - 1, _T("<x 20><c #808080>%s</c>"), item.full_filename.GetData());
#endif
			full_filename->SetShowHtml(true);
			full_filename->SetText(szBuf);
		}
	}

	pListElement->SetFixedHeight(kPlayListItemNormalHeight);
...
bool CPlayList::RemoveNode(Node* node)
{
...
void CPlayList::InvalidateNodes(Node *node)
{
	if (!root_node_)
		return;
	...
	for (int i = 0; i < root_node_->num_children(); ++i)
	{
		...
		CControlUI* control = GetItemAt(child->data().list_elment_->GetIndex());
		
		CContainerUI* logo_container = static_cast<CContainerUI*>(paint_manager_.FindSubControlByName(control, kLogoContainerControlName));
		bool visible = false;
		...
		logo_container->SetVisible(visible);  
...
void CPlayList::SetChildVisible(Node* node, bool visible)
{
	...
	if (node->data().has_child_)
	{
		...
		CLabelUI* file_name = static_cast<CLabelUI*>(paint_manager_.FindSubControlByName(node->data().list_elment_, kNickNameControlName));
		if (file_name != NULL)
		{
			file_name->SetShowHtml(true);
			file_name->SetText(html_text.GetData());
...
bool CPlayList::CanExpand(Node* node) const
{
	if (!node || node == root_node_)
		return false;
...
bool CPlayList::SelectItem(int iIndex, bool bTakeFocus)
{
    ...
    if( m_iCurSel >= 0 ) 
	{
        ...
        if( pControl != NULL) {
            ...
            if( pListItem != NULL )
			{
				CListContainerElementUI *pPlayListItem = static_cast<CListContainerElementUI*>(pControl);
				Node* node = (Node*)pControl->GetTag();
				if ((pPlayListItem != NULL) && (node != NULL) && !node->folder())
				{
					CContainerUI* pOperatorPannel = static_cast<CContainerUI*>(paint_manager_.FindSubControlByName(pPlayListItem, kLogoButtonControlName));
					if (pOperatorPannel != NULL)
					{
						//pOperatorPannel->SetVisible(false); 
					    pPlayListItem->SetFixedHeight(kPlayListItemNormalHeight);
					}
				}
	...
	if( pControl != NULL) {
		...
		if ((pPlayListItem != NULL) && (node != NULL) && !node->folder())
		{
			...
			if (pOperatorPannel != NULL)
			{
			    ...
			    pPlayListItem->SetFixedHeight(kPlayListItemSelectedHeight);
